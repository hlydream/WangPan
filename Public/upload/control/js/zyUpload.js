		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
				    ...
				    html += '							<img class="upload_image" src="../Public/upload/control/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
...
//				}
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				// 处理不同类型文件代表的图标
				var fileImgSrc = "../Public/upload/control/images/fileType/";
				...
				}else if(file.name.indexOf("docx") >= 0){
					fileImgSrc = fileImgSrc + "doc.png";
			...
			this.createPopupPlug = function(imgSrc, index){
				...
				$("body").zyPopup({
					...
					onTailor   :   function(val){     // 回调返回裁剪的坐标和宽高的值
						...
						$.getScript("../Public/upload/jquery.json-2.4.js", function(){
							...
							if($(".file_edit").length>0){
								if($("head").html().indexOf("zyPopup")<0){  // 代表没有加载过js和css文件
									...
									$("<link>").attr({ rel: "stylesheet",
								        type: "text/css",
								        href: "../Public/upload/zyPopup/css/zyPopup.css"
								    }).appendTo("head");
									$.getScript("../Public/upload/zyPopup/js/zyPopup.js", function(){
										// 编辑方法
